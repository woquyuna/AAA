...
class StreamPETRHeadPTH(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is StreamPETRHeadPTH):
        ...
        super(StreamPETRHeadPTH, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def pre_update_memory(self, data):
        ...
        if self.memory_embedding is None:
            ...
            self.memory_timestamp = self.memory_timestamp.to(torch.float32)     # mem_timestamp in model keeps delta,fp32 is enough for presion?
    ...
    def post_update_memory(self, data, rec_ego_pose, all_cls_scores, all_bbox_preds, outs_dec, mask_dict):
        ...
        # print(self.memory_reference_point.shape, data['ego_pose'].shape)
    ...
    def temporal_alignment(self, query_pos, tgt, reference_points):
        ...
        if self.with_ego_pos:
            ...
            # print("*" * 25, "dump constant", "*" * 25)
            print("tgt:", tgt.shape)
            tgt.cpu().detach().numpy().tofile(self.c_path + '/tgt_1x300x256.bin')

            # print("query_pos:", query_pos.shape)
            # query_pos.cpu().detach().numpy().tofile('/mnt/data/userdata/hjj/streampetr_data/query_pos_1x300x256.bin')
        ...
        query_pos_delta = self.time_embedding(pos2posemb1d(torch.zeros_like(reference_points[...,:1])))
        # print("*" * 25, "dump constant", "*" * 25)
        # print("query_pos_delta:", query_pos_delta.shape)
        # query_pos_delta.cpu().detach().numpy().tofile('/mnt/data/userdata/hjj/streampetr_data/query_pos_delta_1x300x256.bin')

        query_pos += query_pos_delta

        # print("*" * 25, "dump constant", "*" * 25)
        print("query_pos:", query_pos.shape)
        query_pos.cpu().detach().numpy().tofile(self.c_path + '/query_pos_1x300x256.bin')
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is StreamPETRHeadPTH:
            convert_dict = {
    ...
    def forward(self, memory_center, img_metas, topk_indexes=None,  **data):
        ...
        self.c_path = data['c_path']
        self.pre_update_memory(data)

        x = data['img_feats']
        if torch.onnx.is_in_onnx_export():
            B, N, C, H, W = 1, 6, 256, 16, 44
        else:
            B, N, C, H, W = x.shape
        ...
        pos_embed, cone = self.position_embeding_pth(data, memory_center, topk_indexes, img_metas)
        ...
        tmp_rec_ego_pose = rec_ego_pose.clone()
        ...
        all_bbox_preds_03 = (all_bbox_preds[..., 0:3] * (self.pc_range[3:6] - self.pc_range[0:3]) + self.pc_range[0:3])
        all_bbox_preds_3_ = all_bbox_preds[..., 3:]
        all_bbox_preds = torch.concat([all_bbox_preds_03, all_bbox_preds_3_], dim=-1)

        self.memory_timestamp = self.memory_timestamp.to(torch.float64)
        # for dump memory
        dump_mem_embed = self.memory_embedding.clone()
        dump_mem_timestamp = self.memory_timestamp.clone()
        dump_mem_egopose = self.memory_egopose.clone()
        dump_mem_ref_point = self.memory_reference_point.clone()
        dump_mem_velo = self.memory_velo.clone()
        ...
        if mask_dict and mask_dict['pad_size'] > 0:
            ...
            outs = {
                ...
                'mem_embedding': dump_mem_embed,
                'mem_timestamp': dump_mem_timestamp,
                'mem_egopose': dump_mem_egopose,
                'mem_ref_point': dump_mem_ref_point,
                'mem_velo': dump_mem_velo
            }
        else:
            outs = {
                ...
                'mem_ref_point': dump_mem_ref_point,
                'mem_velo': dump_mem_velo,
                # 'rec_ego_pose': tmp_rec_ego_pose,
                'outs_dec': outs_dec[-1],
                'post_mem_embedding': self.memory_embedding,
                'post_mem_timestamp': self.memory_timestamp,
                'post_mem_egopose': self.memory_egopose,
                'post_mem_ref_point': self.memory_reference_point,
                'post_mem_velo': self.memory_velo,
            }
    ...
    def position_embeding_pth(self, data, memory_centers, topk_indexes, img_metas):
        eps = 1e-5
        BN, H, W, _ = memory_centers.shape
        B = data['intrinsics'].size(0)
        ...
        coords.cpu().detach().numpy().tofile(self.c_path + '/coords_1x4224x64x4x1.bin')

        img2lidars = data['lidar2img'].inverse()
        img2lidars = img2lidars.view(BN, 1, 1, 4, 4).repeat(1, H * W, D, 1, 1).view(B, LEN, D, 4, 4)
        img2lidars = topk_gather(img2lidars, topk_indexes)
        # print("*"*50)
        # print("img2lidars:", img2lidars.shape)
        # print(img2lidars)
        # img2lidars.cpu().detach().numpy().tofile('/mnt/data/userdata/hjj/streampetr_data/img2lidars_1x4224x64x4x4.bin')
        coords3d = torch.matmul(img2lidars, coords).squeeze(-1)[..., :3]
        ...
        # print(intrinsic)
        # intrinsic.cpu().detach().numpy().tofile('/mnt/data/userdata/hjj/streampetr_data/intrinsic_1x4224x2.bin')
