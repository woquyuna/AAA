...
class StreamPETRHeadONNX(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is StreamPETRHeadONNX):
        ...
        super(StreamPETRHeadONNX, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    # def memory_refresh(memory, prev_exist, view_shape=None):
    ...
    def reset_memory(self):
        ...
        B = 1
        self.memory_embedding = torch.zeros((B, self.memory_len, self.embed_dims), dtype=torch.float32)
        self.memory_reference_point = torch.zeros((B, self.memory_len, 3), dtype=torch.float32)
        self.memory_timestamp = torch.zeros((B, self.memory_len, 1), dtype=torch.float32)
        self.memory_egopose = torch.zeros((B, self.memory_len, 4, 4), dtype=torch.float32)
        self.memory_velo = torch.zeros((B, self.memory_len, 2), dtype=torch.float32)

    def pre_update_memory_onnx(self, data):
        x = data['prev_exists']
        B = 1
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is StreamPETRHeadONNX:
            convert_dict = {
    ...
    def forward(self, img_metas, topk_indexes=None, **data):
        """Forward function.
        Args:
        ...
        self.pre_update_memory_onnx(data)
        ...
        if torch.onnx.is_in_onnx_export():
            pos_embed, cone = self.position_embeding_onnx(data)
        else:
        ...
        tgt, query_pos, reference_points, temp_memory, temp_pos, rec_ego_pose = self.temporal_alignment_onnx(data, reference_points)
        ...
        for lvl in range(outs_dec.shape[0]):
            ...
            print(tmp.shape)
            if torch.onnx.is_in_onnx_export():
                tmp = tmp.reshape(1, self.num_query + self.num_propagated, 10)
                outputs_class = outputs_class.reshape(1, self.num_query + self.num_propagated, self.num_classes)
        ...
        if mask_dict and mask_dict['pad_size'] > 0:
            ...
            outs = {
                ...
                'mem_embedding': self.memory_embedding,
                'mem_timestamp': self.memory_timestamp,
                'mem_egopose': self.memory_egopose,
                'mem_ref_point': self.memory_reference_point,
                'mem_velo': self.memory_velo,
                # 'rec_ego_pose': rec_ego_pose,
                'outs_dec': outs_dec[-1],
            }
        else:
            print(self.memory_timestamp.dtype)
            outs = {
    ...
    def position_embeding_onnx(self, data):
        BN, H, W, B = 6, 16, 44, 1
        D = 64
    ...
    def temporal_alignment_onnx(self, data, reference_points):
        B = 1
        ...
        # query_pos += data['query_pos_delta']
        # print("query_pos:", query_pos)
        # query_pos += self.time_embedding(pos2posemb1d(torch.zeros_like(reference_points[..., :1])))
        # pos2posemb1d_ = pos2posemb1d(self.memory_timestamp).float()
        # print("pos2posemb1d:", pos2posemb1d_, pos2posemb1d_.dtype)
