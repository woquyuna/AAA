...
class Petr3DONNX(MVXTwoStageDetector):
    """Petr3D."""
    ...
    def __init__(self,
                 ...
                 single_test=False,
                 pretrained=None):
        super(Petr3DONNX, self).__init__(pts_voxel_layer, pts_voxel_encoder,
        ...
        for var, name in [(img_metas, 'img_metas')]:
            if not isinstance(var, list):
                raise TypeError('{} must be a list, but got {}'.format(
                    name, type(var)))
        for key in data:
            if (key == 'lidar2img' or key == 'intrinsics' or
                    key == 'extrinsics' or key == 'img_timestamp'):
                data[key] = data[key][0].data[0][0].unsqueeze(0).to('cuda:0')
            else:
                data[key] = data[key][0]
    ...
    def simple_test_pts(self, img_metas, **data):
        ...
        outs = self.pts_bbox_head(img_metas, None, **data)
        return outs
    ...
    def simple_test(self, img_metas, **data):
        ...
        print("img_feats:", data['img_feats'])
